...
/// Target chunk size in bytes (64 KiB)
pub const TargetChunkSize: usize = 65536;
...
pub const XorbChunkHeaderSize: usize = 8;

/// Xorb format version
pub const XorbVersion: u8 = 0;
...
/// MDB shard footer size in bytes
pub const MdbFooterSize: usize = 200;

/// MDB shard header version
pub const MdbHeaderVersion: u64 = 2;

/// MDB shard footer version
pub const MdbFooterVersion: u64 = 1;
...
test "chunking constants are valid" {
    ...
    try std.testing.expect(TargetChunkSize < MaxChunkSize);
    try std.testing.expect(MinChunkSize == 8192);
    try std.testing.expect(TargetChunkSize == 65536);
    try std.testing.expect(MaxChunkSize == 131072);
}

test "gearhash table has correct size" {
    try std.testing.expectEqual(256, GearHashTable.len);
}

test "gearhash mask is correct" {
    try std.testing.expectEqual(@as(u64, 0xFFFF000000000000), GearHashMask);
}

test "blake3 keys have correct size" {
    try std.testing.expectEqual(32, DataKey.len);
    try std.testing.expectEqual(32, InternalNodeKey.len);
    try std.testing.expectEqual(32, FileHashKey.len);
    try std.testing.expectEqual(32, VerificationKey.len);
}
...
test "xorb constants are valid" {
    try std.testing.expectEqual(64 * 1024 * 1024, MaxXorbSize);
    try std.testing.expectEqual(8, XorbChunkHeaderSize);
    try std.testing.expectEqual(0, XorbVersion);
}

test "shard constants are valid" {
    try std.testing.expectEqual(48, MdbHeaderSize);
    try std.testing.expectEqual(200, MdbFooterSize);
    try std.testing.expectEqual(2, MdbHeaderVersion);
    try std.testing.expectEqual(1, MdbFooterVersion);
    try std.testing.expectEqual(48, MdbEntrySize);
}
